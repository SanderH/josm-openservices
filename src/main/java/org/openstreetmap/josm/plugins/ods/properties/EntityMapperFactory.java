...
public class EntityMapperFactory<T1, T2> {
    private final OdsEntityType<T1> sourceType;
    private final OdsEntityType<T2> targetType;
    private EntityFactory<T2> factory;
    private final List<EntityAttributeMapper<T1, T2>> attributeMappers = new LinkedList<>();
    private final List<ChildMapper<T1, T2>> childAttributeMappers = new LinkedList<>();
    ...
    public EntityMapperFactory(OdsEntityType<T1> sourceType,
    ...
    private <A1, A2> EntityAttributeMapper<T1, T2> buildTransformingAttributeMapper(AttributeMapping attributeMapping) {
        ...
        if (targetAttributeHandler == null) {
            addIssue(I18n.tr("The attribute ''{0}'' doesn''t exist in the target type ''{1}''", targetAttr, targetType));
        }
        if (sourceAttributeHandler == null || targetAttributeHandler == null) {
            return null;
        }
