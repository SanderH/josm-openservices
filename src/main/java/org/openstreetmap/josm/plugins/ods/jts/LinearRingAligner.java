...
public class LinearRingAligner {
    ...
    private void fix(SegmentIterator _it1, SegmentIterator _it2) {
        _it1.reset(0);
        while (_it1.hasNext()) {
            Coordinate c1 = _it1.next().p0;
            _it2.reset(0);
            while (_it2.hasNext()) {
                LineSegment ls2 = _it2.next();
                if (ls2.distance(c1) < tolerance) {
                    if (ls2.p0.distance(c1) < tolerance) {
                        _it2.updateStartPoint(c1);
                    }
                    else if (ls2.p1.distance(c1) < tolerance) {
                        _it2.updateEndPoint(c1);
                    }
                    else {
                        _it2.snap(c1, tolerance);
                    }
                }
            }
        }
